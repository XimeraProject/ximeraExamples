...
\fi
  ...
  \ifdefined\HCode
    \HCode{<h1> Command: \string\graph[#1]}\detokenize{#2}\HCode{  </h1> }
  \else
...
\ExplSyntaxOn
% Define a macro that safely stores the argument in a token list
\NewDocumentCommand{\xmgraph}{ O{} m }{
    % #1 = optional key-value options
    % #2 = the expression (can include \frac)
    \tl_set:Nn \l_tmpa_tl { #2 } % store argument literally
    \graph[#1]{\l_tmpa_tl}       % pass it safely to \graph
}
\ExplSyntaxOff
...
\def\xmGraphShowCommand{true}
 ...
 {
   \def\xmGraphShowTikZ{true}
  %  \graph[xmGraphShowTikZ]{x^2/2}
   \graph{x^2/2}
 }
...
%  }
 ...
 {
   \def\xmGraphShowTikZ{true}
   \let\xmGraphShowDesmos\undeFiNeD
    \ifdefined\xmGraphShowDesmos  
    BROLLLL 
    \typeout{BROLLLL tmpstyle} 
    \fi
  \graph[dashed,blue,very thick]{x^2/2}
  % \graph{x^2/2}
  }

 \graph[panel]{x^2/2}

 \graph[panel]{1/x^2, 1/x^4, 1/x^6, (1,1), (1,-1)}
...
%  \graph[panel,xmin=-2.5, xmax=2.5,ymin=-0.5,ymax=8]{$\frac{1}{x^2}$, $\frac{1}{x^4}$, $\frac{1}{x^6}$, (1,1), (1,-1)}
 ...
 \graph[xmin=-5,xmax=5]{x^2/2}

 \graph[xmin=-5,xmax=5,xmDesmosHeight=1000, xmDesmosWidth=1000]{x^2/2}
